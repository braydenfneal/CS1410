 ...
 * 
 * @author Profs. David E. Johnson & Brayden Neal & Travis Martin
 *
 */
public class MovieReviewPredictor {
	...
	public static HashMap<String, Double> computeWordValues(ArrayList<String> reviews) {
		...
		for (String review : reviews) {
			String[] words = review.substring(2).split(" ");
			for (String word : words) {
				double avgScores = (computeWordScores.get(word) / computeWordCount.get(word));

				if (avgScores < 1.75 || avgScores > 2.25) {
					computeWordValues.put(word, avgScores);
				}
			}
		}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (String line : lines) {
			...
			/*
			 * I think like a lot of the assignments that we have been doing is that it
			 * could be improved by a whole lot. I think as a start it is good, because it
			 * gives a general basis of an average rating score, so I still think it could
			 * be usable.
			 */
		}
